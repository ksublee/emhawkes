...
The terms $\lambda_{ij}$ are referred to as lambda components, and `lambda0` represents $\lambda_{ij}(0)`. 
The parameter `lambda_component0` can be omitted in this model, in which case internally determined initial values will be used.
...
For illustration purposes, we set `hspec0 <- hspec2`. Since the true parameter values are unknown in practical applications, these initial guesses are used. 
...
In practical scenarios, the parameter values are usually unknown, 
so the model may be estimated before computing the residual processes.
...
set.seed(903)
# run simulation
res_sy <- hsim(h_sy, size = 4000, lambda_component0 = matrix(rep(1, 2 * 4), nrow=2))
summary(res_sy)
```
...
```{r, warning=FALSE}
set.seed(620)
res_impact <- hsim(hi, size=2000, lambda_component0 = matrix(rep(0.1,4), nrow=2))
summary(res_impact)
```
...
```{r}
set.seed(1107)
hr_ba <- hsim(h_ba, size=4000, lambda_component0 = matrix(rep(1, 16), 4))
summary(hr_ba)
```
...
hist(dh1_real$inter_arrival[dh1_real$inter_arrival < 3], breaks=50)
```
...
hist(infer_res_dh1$res_process1, breaks = 100, probability = TRUE)

x <- seq(0, 8, 0.1)
lines(x, dtzexp(x, a = a, ell = ell), col = 'red')
```
...
hist(duh2_real$inter_arrival[duh2_real$inter_arrival < 5], breaks=50)
```
...
min_res <- c(infer_res_duh2$min_res_process1,infer_res_duh2$min_res_process2)

sorted_min_res <- sort(min_res)

u <- seq(0, 1, length.out =  length(min_res) + 2)[-c(1, length(min_res) + 2)]
v <-  1 - (1-u)^(1/2)

U <- sort(runif(10000))

res <- approx(x = v, y = sorted_min_res, xout = U, rule =2)$y
...
hspec2_d <- new("hspec", mu=mu2_d, alpha=alpha2_d, beta=beta2_d,
              rresidual = function(size) rtzexp(n=size, a = a, ell = ell),
              dresidual = function(x, param = c(a = a, ell = ell))
                dtzexp(x, a = param["a"], ell = param["ell"]),
              presidual = function(q, param = c(a = a, ell = ell))
...
```{r, fig.height=4, fig.width=6}
set.seed(1107)
dh2_real <- hsim(hspec2_d, size = 2000)
hist(dh2_real$inter_arrival[dh2_real$inter_arrival < 5], breaks=50)
```


```{r, message=FALSE}
infer_res_dh2 <- logLik(hspec2_d, inter_arrival = dh2_real$inter_arrival, type = dh2_real$type, infer = TRUE)
infer_res_dh2$rambda_component[1:10, ]
```
...
```{r, warning=FALSE}
mle_dh <- hfit(hspec2_d, inter_arrival = dh2_real$inter_arrival, type = dh2_real$type,
...
mu_dh_e <- matrix(c(coef(mle_dh)[["mu1"]], coef(mle_dh)[["mu1"]]), nrow=2)
alpha_dh_e <- matrix(c(coef(mle_dh)[["alpha1.1"]], coef(mle_dh)[["alpha1.2"]],
                       coef(mle_dh)[["alpha1.2"]], coef(mle_dh)[["alpha1.1"]]), nrow=2, byrow=TRUE)
beta_dh_e <- matrix(rep(coef(mle_dh)[["beta1.1"]], 4), nrow=2)
                
a_e <- coef(mle_dh)[["a"]]; ell_e <- coef(mle_dh)[["ell"]]

hspec2_d_e <- new("hspec", mu=mu_dh_e, alpha=alpha_dh_e, beta=beta_dh_e,
              dresidual = function(x, param = c(a = a_e, ell = ell_e))
                dtzexp(x, a = param["a"], ell = param["ell"]),
              presidual = function(q, param = c(a = a_e, ell = ell_e))
                  ptzexp(q, a = param["a"], ell = param["ell"]))
...
alpha2_d <- matrix(c(0.5, 0.5, 0.5, 0.5), nrow = 2, byrow = TRUE)
...
duh2_real <- hsim(hspec2_du, size = 3000)
...
U <- runif(length(min_res))

res <- approx(x = v, y = sorted_min_res, xout = U)$y
hist(res)
```
