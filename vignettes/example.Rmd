...
```{r}
# run simulation
res_sy <- hsim(h_sy, size = 5000, lambda_component0 = matrix(rep(1, 2 * 4), nrow=2))
summary(res_sy)
```
...
$$
 \lambda(t) = \mu + \int_{(-\infty,t)\times E} h(t, u)M(du \times dz)
$$

where the kernel $h$ is represented by

$$
h(t) = (\alpha + g(t, z))\Gamma(t),
$$
...
In this model, we use a system of counting processes with the corresponding conditional intensities:
...
where \$p(t)\$ is the mid-price and $L(t) \in \{ 0, 1, 2, \cdots \}$ is the absolute level of the bid-ask spread with $L(t)=0$ implying the minimum level.
